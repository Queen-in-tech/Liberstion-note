...
const AuthProvider = ({ children }) => {
  ...
  useEffect(() => {
    const getPostLikedByUser = async () => {
      const likesQuery = query(collection(db, "users", user.uid, "dailyData"));
      ...
      likedPostsSnapshots.map((snapshot) =>
        snapshot.docs.map((doc) => likedPosts.push(doc.id))
      );
  ...
  const checkPostLikes = () => {
    ...
    const postLikes = postsData.map((post) => {
      const initCount = allPostLiked.filter(
        (likedId) => likedId === post.id
      ).length;
  ...
  const likePost = async (post) => {
    ...
    const dailyDataRef = doc(
      collection(db, "users", user.uid, "dailyData", revToday, "likes"),
      post.id
    );
