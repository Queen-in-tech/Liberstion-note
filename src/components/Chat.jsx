...
const Chat = ({ otherUser, setStartChat, roomId }) => {
  ...
  const otherUserName = otherUser.firstPersonName
    ? otherUser.firstPersonName
    : otherUser.secondPersonName || otherUser.username;
  const otherPhoto = otherUser.firstPersonPhoto
    ? otherUser.firstPersonPhoto
    : otherUser.secondPersonPhoto || otherUser.photoURL;
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      if (querySnapshot.exists()) {
        ...
        const initialData = {
          messages: [{ message, time: new Date(), from: user.displayName }],
        };

        await setDoc(q, initialData).then("Daily data saved successfully!");
      }
    } catch (err) {
      console.log(err);
    }
  ...
  return (
    <div className="absolute top-0 bottom-0 right-0 left-0 z-10 md:rounded-2xl bg-dGreen  p-4">
      <div>
        ...
        <div className="flex flex-col gap-2 mt-3 h-[61vh] overflow-y-scroll scrollbar">
          {messageData &&
            messageData.map((message, index) => {
              return (
                <div
                  ...
                  ref={index === messageData.length - 1 ? scrollRef : null}>
                  <div
                    className={`p-2 w-auto max-w-[70%] ${
                      message.from === user.displayName
                        ? "bg-dBlue rounded-l-lg rounded-tr-lg"
                        : "bg-white/40 rounded-r-lg rounded-tl-lg"
                    }`}>
      ...
      {showEmoji && (
        <div className="text-sm text-white absolute bg-white bottom-10 z-30">
          <Picker
