  ...
  return (
    <div className="">
      <div className="flex justify-between items-center">
        <AiOutlineArrowLeft
          className="text-dBlue text-lg md:text-white block md:hidden"
      ...
      {chatList ? (
        <div className="py-3 flex flex-col gap-4">
          {chatList
            .sort(
              (a, b) =>
                b[Object.keys(b)].lastMessageTime -
                a[Object.keys(a)].lastMessageTime
            )
            .map((chat, index) => {
              for (const key in chat) {
                const doc = chat[key];
                const person =
                  doc.firstPerson.firstPersonName !== user.displayName
                    ? doc.firstPerson
                    : doc.secondPerson;
                const personName =
                  doc.firstPerson.firstPersonName !== user.displayName
                    ? doc.firstPerson.firstPersonName
                    : doc.secondPerson.secondPersonName;
                const personPhoto =
                  doc.firstPerson.firstPersonPhoto !== user.photoURL
                    ? doc.firstPerson.firstPersonPhoto
                    : doc.secondPerson.secondPersonPhoto;
                ...
                return (
                  <div
                    ...
                    <div className="flex gap-2 items-center cursor-pointer">
                      ...
                      <div>
                        <p className="text-dBlue md:text-white capitalize flex-row items-center justify-between">
                          {personName}
                        </p>
                        {doc.lastMessage && (
                          <p className="text-dBlue/50 md:text-white/80 text-sm">
                            {doc.lastMessageBy === user.displayName
                              ? "You:  "
                              : ""}
                            {doc.lastMessage}
                          </p>
                    ...
                    <span className="lowercase text-sm text-dBlue/50 md:text-white/80 text-center">{`${
                      doc.lastMessageTime
                        ? renderDateTime(doc.lastMessageTime)
                        : ""
                    }`}</span>
                  </div>
                );
      ...
      {startChat && (
        <Chat otherUser={otherUser} roomId={key} setStartChat={setStartChat} />
      )}
    </div>
  );
};
