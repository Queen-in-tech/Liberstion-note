...
const MakeAPost = () => {
  const { mood, setMakePostMobile } = useContext(AuthContext);
  ...
  const saveDailyData = async () => {
    try {
      ...
      if (docSnapshot.exists()) {
        ...
        await setDoc(dailyDataRef, initialData).then(
      ...
      <div className="mx-3 md:mx-10 bg-white rounded-xl relative">
        <div className="pt-4 md:border-b-dGreen border-b-[1.5px] rounded-b-xl">
          <div className="flex justify-between px-2 pt-2 pb-4 md:hidden">
            <p className="text-dBlue" onClick={() => setMakePostMobile(false)}>
              Cancel
            </p>
            <button
              className={`bg-dGreen px-5 py-1 md:hidden text-white text-sm rounded-2xl ${
                postText.length > 300 && "bg-[#94f7c8] cursor-not-allowed"
              }`}
              onClick={handleClick}>
              Post
            </button>
          ...
          <div className="md:min-h-[80px] h-auto">
            {!postText && (
              <span className="absolute text-gray-400 pointer-events-none p-5">
              ...
              {imgUrl &&
                imgUrl.map((url, index) => (
                  <div className="relative">
                    ...
                    />
                    <FaTimes
                      className="absolute top-1 left-2 bg-black text-xl p-1 cursor-pointer rounded-full text-white "
            ...
            <div className="flex justify-between items-center pl-3 pr-5">
              <div className="">
                <button
                  ...
                  <MdOutlinePrivacyTip />{" "}
                  <span>{privacySettings} can see this</span> <AiOutlineDown />{" "}
                </button>
              </div>
              ...
              <div className="p-2 flex gap-2 items-center">
                <p className={`text-[13px] ${textCountClass}`}>{textCount}</p>
                <VscSmiley
                ...
                />
                <button
                  className={`bg-dGreen px-5 py-1 hidden md:block text-white text-sm rounded-2xl ${
                    postText.length > 300 && "bg-[#94f7c8] cursor-not-allowed"
                  }`}
                  onClick={handleClick}>
                  Post
        ...
        {showEmoji && (
          <div className="text-sm text-dGreen absolute bg-white top-[105px] z-50">
            <Picker
        ...
        {privacy && (
          <div className="text-sm text-dGreen absolute bg-white top-[105px] rounded-xl z-50">
            <div className="flex flex-col p-4">
              <p
                className="hover:shadow-xl hover:rounded-b-lg cursor-pointer py-2"
                onClick={() => {
              ...
              <p
                className="hover:shadow-lg hover:rounded-b-lg cursor-pointer py-2"
                onClick={() => {
