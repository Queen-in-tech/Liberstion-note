...
const OthersActivities = () => {
  ...
  useEffect( () => {
    ...
    querySnapshot.forEach( async (userDoc) => {
      ...
      const dailyDataRef = collection(db, "users", userUid, "dailyData");
      const dailyDataSnapshot = await getDocs(dailyDataRef);

      
      dailyDataSnapshot.forEach((dailyDataDoc) => {
        const dailyData = dailyDataDoc.data();
      jsonData.push(dailyData);
      jsonData.forEach((dailyData) => {
      if (dailyData.posts) {
      postsArray.push(...dailyData.posts);
